...
distance: $minf[]$, and the maximum amount of flights with the minimum distance to reach that node. For every node, $v$, we take into consideration all of 
it's neighbors, $u$. If we can reach the $u$ in a shorter distance than it's current minimum, we update the distance and reset $num[u]$, $minf[u]$, and $maxf[u]$. 
We also have to take into consideration if we can reach $u$ with an equivalent distant. If so, we update:
